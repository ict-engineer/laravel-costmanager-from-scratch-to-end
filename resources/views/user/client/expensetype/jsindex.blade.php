...
<script type="text/javascript">
  ...
  $(document).ready(function () {
    ...
    $('#newexpensetype').click(function () {
        
        $('#saveBtn').html("Add");
        $('#typeid').val('');
        $('#sendForm').trigger("reset");
        $('#modalheader').html("New Expense Type");
    ...
    $('body').on('click', '.editexpensetype', function () {
        var id = $(this).data('id');
        $.get("{{ route('user.clientexpensetypes.index') }}" +'/' + id +'/edit', function (data) {
            $('#saveBtn').html("Save");
            $('#modalheader').html("Edit Expense Type");
            $('#ajaxModel').modal()[0].M_Modal.options.dismissible = false;
            $('#ajaxModel').modal('open');
            $('#typeid').val(data.id);
            $('#name').val(data.name);
   ...
   if (i == 1)
        var table = $('#page-length-option').DataTable({
            ...
            info: true,
            searching: true,
            ajax: "{{ route('user.clientexpensetypes.index') }}",
            "language": {
                "paginate": {
    ...
    $("body").on("click",".remove-expensetype",function(){
        ...
        swal({
            ...
            if(result){
                $.ajax({
                    type: "DELETE",
                    url: "{{ route('user.clientexpensetypes.store') }}"+'/'+id,
   ...
   $('body').on('click', '#saveBtn', function (e) {
        ...
        $.ajax({
            ...
            url: "{{ route('user.clientexpensetypes.store') }}",
            type: "POST",
            dataType: 'json',
            success: function (data) {
                $(".error").html('');
